...
function startStream(ip) {
  // Remove qualquer porta que o usuario tenha digitado, e força :81 pro stream
...
async function pollStatus() {
  try {
    const res = await fetch('/info', { signal: AbortSignal.timeout(5000) });
    ...
    if (state.connected) {
      ...
      addLogEntry('denied', 'Conexão perdida', 'ESP32 ficou offline');
    }
  }
}
