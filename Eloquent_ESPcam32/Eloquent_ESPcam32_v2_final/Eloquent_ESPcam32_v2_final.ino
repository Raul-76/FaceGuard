...
// ---------------- BOTÃO ----------------
const int iniciarReconhecimento = 21; //botao usado para iniciar o reconhecimento
//SWITCH
...
// ==========================================
// 1. BOOT / INICIALIZAÇÃO DO SISTEMA
// Mantido. Rápido e indica que o ESP32 conectou e a câmera está pronta.
// ==========================================
...
// ==========================================
// 2. ALARME / TAMPER (Violação do equipamento)
// Mantido. Padrão internacional de alerta.
// ==========================================
...
// ==========================================
// 3. PORTÃO ABERTO (Acesso Permitido)
// Padrão de leitora HID/Intelbras: Dois bipes curtos e agudos, o segundo mais alto.
// Passa a sensação de "Catraca Liberada" ou "Trava Aberta".
// ==========================================
const Nota somBoot[] = {
  {1800, 80},  // Bipe curto e agudo
  {0, 40},     // Pausa quase imperceptível
  {2300, 200}, // Bipe mais alto e ligeiramente mais longo
  {0, 0}
};

// ==========================================
// 5. ACESSO NEGADO (Rosto não reconhecido)
// Padrão industrial de erro: Um bipe longo, grave e plano. 
// Seco, sem ser escandaloso, mas inconfundível com o de abertura.
// ==========================================
const Nota somAcessoNegado[] = {
  {600, 150}, // Ataque imediato
  {450, 150}, // Transição rápida
  {300, 300}, // Finalização grave e seca
  {0, 0}
};
...
void tocarMelodia(const Nota melodia[]) {
  ...
  while (melodia[i].frequencia != 0 || melodia[i].duracao != 0) {
    if (melodia[i].frequencia == 0) {
      ...
      // Para bipes industriais, um atraso exato deixa o som mais seco ("crisp")
...
#define PISO_SIM      0.92f    // (D) sim minimo pra um frame virar voto valido
...
#define MIN_SHARP        3000  // nitidez minima (bytes do JPEG). ~85% do teu pico focado.
...
/**
 * Finaliza a tentativa: garante que o LED AZUL apaga e sinaliza o resultado.
 * pino = pinoVerde  (aprovado)
 * pino = pinoVermelho (negado / erro / timeout)
 *
 * Faz sempre a dupla:
 *   analogWrite(pinoAzul, 0)   -> para o PWM residual do pulsaLEDEspera
 *   digitalWrite(pinoAzul, LOW) -> garante nivel logico LOW no pino
 * Sem isso, o ultimo brilho do PWM azul continua "vivo" e se mistura com
 * o vermelho/verde, dando cor errada ou parecendo que o azul nao apagou.
 */
void sinalizaResultado(int pino, const Nota melodia[]) {
...
/**
 * Faz o LED Azul pulsar suavemente usando uma onda senoidal.
 * Nao bloqueia o loop.
 */
void pulsaLEDEspera(){
...
Veredito runTentativa() {
    ...
    while (validos < JANELA_N) {
        ...
        pulsaLEDEspera(); // atualiza o brilho do LED azul a cada iteracao

        // ----- timeout: rosto nao aparece / some -----
        if (millis() - t0 > TIMEOUT_MS) {
            Serial.printf(">> EXPIROU (so %d/%d validos)\n", validos, JANELA_N);
            sinalizaResultado(pinoVermelho, somAcessoNegado);   // <-- apaga azul + acende vermelho
            return EXPIROU;
        }
        ...
        if (votosFavor >= VOTOS_K) {
            Serial.printf(">> APROVADO (early-exit: %d votos em %d frames)\n",
                          votosFavor, validos);
            sinalizaResultado(pinoVerde, somPortaAberta);      // <-- apaga azul + acende verde
            return APROVADO;
        }
...
void loop() {
    ...
    esp_sleep_enable_ext0_wakeup(pinoDeepSleep, 1);  // acorda quando o pino voltar a HIGH
    ...
    // borda de descida = passou de HIGH (solto) pra LOW (apertado)
...
void enrollMultiplo(int alvo) {
    ...
    while (ok < alvo) {
        ...
        for (int k = 0; k < 20; k++) {
            ...
            // Em vez de delay(100); fazemos um loop de 100ms atualizando o LED
