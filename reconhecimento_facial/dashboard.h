...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    .log-entry.granted-entry .log-type {
      color: var(--green);
    }

    .log-entry.denied-entry .log-type {
      color: var(--red);
    }
    ...
    .granted-card .card-num {
      color: var(--green);
    }

    .denied-card .card-num {
      color: var(--red);
    }
    ...
    /* ===================== NEW CUSTOM CSS ===================== */
    .login-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: var(--bg-base); display: flex; align-items: center; justify-content: center; z-index: 10000; transition: opacity 0.5s; }
    .login-box { background: var(--bg-panel); border: 1px solid var(--border); padding: 30px; border-radius: 12px; width: 320px; text-align: center; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
    ...
    .face-item { display: flex; align-items: center; justify-content: space-between; padding: 10px; border-bottom: 1px solid var(--border); }
    .face-item:last-child { border-bottom: none; }
    .face-item-name { flex-grow: 1; padding: 0 10px; }
    .face-item-name input { background: transparent; border: 1px solid transparent; color: var(--text-primary); width: 100%; padding: 5px; font-family: inherit; }
    .face-item-name input:focus { border: 1px solid var(--accent); outline: none; background: var(--bg-input); border-radius: 4px; }
  </style>
</head>
...
<body>
  <div class="login-overlay" id="loginOverlay">
    <div class="login-box">
      <div class="logo-icon" style="margin: 0 auto 15px; color: var(--accent); display:flex; justify-content:center;">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />
          <circle cx="12" cy="12" r="3" />
        </svg>
      </div>
      <h2 style="margin-bottom: 20px; color: var(--text-primary);">FaceGuard Login</h2>
      <div id="loginErrorMsg" style="display:none; color: #ef4444; background: rgba(220, 38, 38, 0.1); padding: 8px; border-radius: 6px; margin-bottom: 15px; border: 1px solid rgba(220, 38, 38, 0.2); font-size: 14px;">Invalid username or password</div>
      <input type="text" id="loginUsername" placeholder="Enter username..." onkeydown="if(event.key === 'Enter') checkLogin()" style="width: 100%; margin-bottom: 10px; background: var(--bg-input); border: 1px solid var(--border); color: var(--text-primary); padding: 10px; border-radius: 6px; outline: none;"/>
      <input type="password" id="loginPassword" placeholder="Enter password..." onkeydown="if(event.key === 'Enter') checkLogin()" style="width: 100%; margin-bottom: 15px; background: var(--bg-input); border: 1px solid var(--border); color: var(--text-primary); padding: 10px; border-radius: 6px; outline: none;"/>
      <button style="width: 100%; padding: 10px; border-radius: 6px; cursor: pointer; background: var(--accent); color: white; border: none; font-weight: 600;" onclick="checkLogin()">Login</button>
    </div>
  </div>
  ...
  <!-- MANAGE FACES MODAL -->
  <div class="modal-backdrop" id="facesModalBackdrop" style="display:none;">
    <div class="modal-box" style="width: 400px; max-height: 80vh; display: flex; flex-direction: column;">
      <div class="modal-header">
        <h3>Manage Faces</h3>
        <button class="modal-close" onclick="closeFacesModal()">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <line x1="18" y1="6" x2="6" y2="18" />
            <line x1="6" y1="6" x2="18" y2="18" />
          </svg>
      ...
      <div class="modal-body" style="overflow-y: auto; flex-grow: 1;">
        <div id="facesListLoading" style="text-align:center; padding:20px; color:var(--text-muted);">Loading...</div>
        <div id="facesListContainer" style="display:none;"></div>
      </div>
      <div class="modal-footer" style="justify-content: space-between;">
        <button class="btn-danger" id="btnDeleteSelectedFaces" onclick="deleteSelectedFaces()" style="display:none; padding:8px 12px; border-radius:6px; cursor:pointer;">Delete Selected</button>
        <div>
          <button class="btn-secondary" onclick="closeFacesModal()">Close</button>
        </div>
      </div>
    </div>
  </div>
